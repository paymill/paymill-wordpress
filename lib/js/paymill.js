if(paymill_pcidss3 == 1){
	...
	jQuery('body').on('updated_checkout',function(){
		...
		paymill.embedFrame(
			frameContainer,
			{
				lang: "en" // other languages: de, fr, it, es, pt
			},
			frameCallback
		);
	});
...
jQuery(document).ready(function () {
	...
	function bridgePreparePayment(){
		...
		if((jQuery('#paymill_form_credit').is(':visible') || (jQuery('#paymill_card_number').length > 0 && jQuery('#paymill_card_number').val() != '')) && (jQuery('#payment_method_paymill').is(':checked') || jQuery('.wgm-second-checkout input[name=payment_method]').val() == 'paymill' || paymill_shop_name == 'cart66')){
			if(paymill_pcidss3 == 1){
